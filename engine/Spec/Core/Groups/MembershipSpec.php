...
class MembershipSpec extends ObjectBehavior
{
    ...
    function let(
        Relationships $db,
        ACL $acl,
        \Opspot\Core\Groups\Notifications $notifications
    )
    {
    ...
    {
        $this->shouldHaveType('Opspot\Core\Groups\Membership');
    }
    ...
    public function it_should_not_join_a_public_group_if_banned(GroupEntity $group, Relationships $db, User $user)
    {
        ...
        $this->setGroup($group);
        $this->setActor($user);
        $this->shouldThrow('\Opspot\Exceptions\GroupOperationException')->duringJoin($user);
    }
    ...
    public function it_should_leave(GroupEntity $group, Relationships $db, User $user, \Opspot\Core\Groups\Notifications $notifications)
    {
        $user->get('guid')->willReturn(1);
        $user->getGroupMembership()->willReturn([]);
        $user->context(Argument::type('string'))->willReturn();
    ...
    public function it_should_kick(GroupEntity $group, Relationships $db, User $user, User $actor, \Opspot\Core\Groups\Notifications $notifications, ACL $acl)
    {
        $user->get('guid')->willReturn(1);
        $user->getGroupMembership()->willReturn([]);
        $user->context(Argument::type('string'))->willReturn();
    ...
    public function it_should_not_kick_if_not_an_owner(GroupEntity $group, Relationships $db, User $user, User $actor,  \Opspot\Core\Groups\Notifications $notifications, ACL $acl)
    {
        $user->get('guid')->willReturn(1);
        $actor->get('guid')->willReturn(2);
        $group->getGuid()->willReturn(50);
        ...
        $this->setGroup($group);
        $this->setActor($actor);
        $this->shouldThrow('\Opspot\Exceptions\GroupOperationException')->duringKick($user);
    }
    ...
    public function it_should_ban(GroupEntity $group, Relationships $db, User $user, User $actor, \Opspot\Core\Groups\Notifications $notifications, ACL $acl)
    {
        $user->get('guid')->willReturn(1);
        $user->getGroupMembership()->willReturn([]);
        $user->context(Argument::type('string'))->willReturn();
    ...
    public function it_should_not_ban_if_not_an_owner(GroupEntity $group, Relationships $db, User $user, User $actor, \Opspot\Core\Groups\Notifications $notifications, ACL $acl)
    {
        $user->get('guid')->willReturn(1);
        $actor->get('guid')->willReturn(2);
        $group->getGuid()->willReturn(50);
        ...
        $this->setGroup($group);
        $this->setActor($actor);
        $this->shouldThrow('\Opspot\Exceptions\GroupOperationException')->duringBan($user);
    }

    public function it_should_not_ban_an_owner(GroupEntity $group, Relationships $db, User $user, User $actor, \Opspot\Core\Groups\Notifications $notifications, ACL $acl)
    {
        $user->get('guid')->willReturn(1);
        $actor->get('guid')->willReturn(2);
        $group->getGuid()->willReturn(50);
    ...
    public function it_should_not_unban_if_not_an_owner(GroupEntity $group, Relationships $db, User $user, User $actor, ACL $acl)
    {
        ...
        $this->setGroup($group);
        $this->setActor($actor);
        $this->shouldThrow('\Opspot\Exceptions\GroupOperationException')->duringUnban($user);
    }
    ...
    public function it_should_not_cancel_request_if_not_an_owner(GroupEntity $group, Relationships $db, User $user, User $actor, ACL $acl)
    {
        ...
        $this->setGroup($group);
        $this->setActor($actor);
        $this->shouldThrow('\Opspot\Exceptions\GroupOperationException')->duringCancelRequest($user);
    }
