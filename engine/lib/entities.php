...
function entity_row_to_elggstar($row, $cache = true) {
    ...
    return \Opspot\Core\Entities::build($row, $cache);
        ...
        switch ($row->type) {
            ...
            case 'widget' :
                $new_entity = new ElggWidget($row);
                        break;
            case 'notification' :
                $new_entity = new \Opspot\Entities\Notification($row);
                break;
            default:
...
function get_entity($guid, $type = 'object') {
    ...
    if(!Opspot\Core\Security\ACL::_()->read($new_entity)){
        ...
        try{
            ...
            if($rows){
                foreach($rows as $guid=>$row){
                    ...
                    if(Opspot\Core\Security\ACL::_()->read($entity))
                        $entities[] = $entity;
                }
            }
        } catch(Exception $e){
