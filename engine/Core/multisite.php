...
class multisite extends base
{
    ...
    public function __construct($domain = null)
    {
        ...
        require_once(__OPSPOT_ROOT__ . '/engine/multi.settings.php');
        ...
        if (strpos($this->domain, '-custdom-001', 0) !== false) {
            $this->domain = str_replace('-custdom-001.ops.doesntexist.com', '', $this->domain);
    ...
    public function load($domain)
    {
        ...
        if (!(isset($row['installed']) || isset($row['enabled'])) && !defined('__OPSPOT_INSTALLING__')) {
        ...
        //if(strpos($CONFIG->wwwroot, '.ops.doesntexist.com') === FALSE)
        //	$CONFIG->wwwroot = "http://$this->host/";

        if (isset($row['dataroot'])) {
            $CONFIG->dataroot = unserialize($row['dataroot']);
        } else {
            $CONFIG->dataroot = "/gluster/data/opspot-multisite/".$keyspace;
        }
