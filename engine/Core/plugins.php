...
class plugins extends base
{
    ...
    {
        self::$path = dirname(__OPSPOT_ROOT__) . "/plugins/";
        $this->load();
    ...
    public static function get($status = 'active')
    {
        ...
        if ($cached = self::getFromCache("plugins:$status")) {
            ...
            $db = new \Opspot\Core\Data\Call('plugin');
    ...
    /**
     * Save to cache
     * @todo make this a generic opspot function
     * @param  string $key
     * @param  array  $data
     * @return bool
     */
    public static function saveToCache($key, $data)
    {
        ...
        global $CONFIG;
        $path = "/tmp/new-opspot/".$CONFIG->cassandra->keyspace;
    ...
    /**
     * Get from cache
     * @todo maek this a generic opspot function
     * @param  string $key
     * @return array|bool
     */
     ...
     */
     public static function factory($guid, $data = null)
     {
         ...
         $class = "\\opspot\\plugin\\$guid\\start";
