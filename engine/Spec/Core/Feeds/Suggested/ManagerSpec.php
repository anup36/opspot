...
namespace Spec\Opspot\Core\Feeds\Suggested;

use Opspot\Core\EntitiesBuilder;
use Opspot\Core\Feeds\Suggested\Manager;
use Opspot\Core\Feeds\Suggested\Repository;
use Opspot\Core\Trending\Aggregates\Aggregate;
use Opspot\Core\Trending\EntityValidator;
...
class ManagerSpec extends ObjectBehavior
{
    ...
    function let(
        Repository $repo,
        \Opspot\Core\Hashtags\Entity\Repository $entityHashtagsRepo,
        Aggregate $agg,
        Aggregate $aggImages,
    ...
    function it_should_collect_and_store_suggested_entities()
    {
        $entities = [
            1 => (new Entity)
                ...
                ->set('owner_guid', 1)
                ->set('type', 'activity')
                ->set('perma_url', 'https://ops.doesntexist.com/blog/view/1000001')
                ->set('message', '#hashtag')
                ->setRating(1),
            2 => (new Entity)
                ->set('guid', 20)
