	...
	Importable // Allow import of data
{
	...
	protected function loadFromGUID($guid){
		$db = new Opspot\Core\Data\Call('entities');
	...
	function canEdit($user_guid = 0) {
		return Opspot\Core\Security\ACL::_()->write($this);
	}
	...
	public function getOwnerEntity($brief = false) {
			...
			if(isset($owner['name']) || (is_object($owner) && $owner->name)){
				return new Opspot\Entities\User($owner);
			}  else {
				if($this->canEdit()){
					//$this->save();
				}
		...
		return new Opspot\Entities\User($this->owner_guid);
	}
	...
	public function save($timebased = true) {
            ...
            if($this->guid){
                ...
                $this->guid = Opspot\Core\Guid::build();
            ...
            if ($result && $timebased) {
                ...
                if (in_array($this->access_id, array(2, -2, 1))) {
                    Opspot\Core\Queue\Client::build()->setQueue("FeedDispatcher")
                        ->send(array(
                            "guid" => $this->guid,
	...
	public function delete($recursive = true) {
		...
		$delete = Opspot\Core\Events\Dispatcher::trigger('delete', $this->type, [ 'entity' => $this ]);
		...
		if ($delete && $this->canEdit()) {
			...
			Opspot\Core\Queue\Client::build()->setQueue("FeedCleanup")
            ->send(array(
                "guid" => $this->guid,
                "owner_guid" => $this->owner_guid,
								"type" => $this->type,
	...
	public function export(){
		...
		$export = array_merge($export, \Opspot\Core\Events\Dispatcher::trigger('export:extender', 'all', array('entity'=>$this), []) ?: []);
		$export = \Opspot\Helpers\Export::sanitize($export);
		return $export;
	}
	 ...
	 */
	 public function feature(){
		...
		$this->featured_id = Opspot\Core\Guid::build();
		...
		if(in_array($this->subtype, array('video', 'image', 'album', 'audio'))){
