...
$offset = "", $output = 'entities') {
	...
	if($output == 'entities'){
	//	$db = new Opspot\Core\Data\Call('user');
...
function get_user_by_code($code) {
	...
	$index = new Opspot\Core\Data\Call('user_index_to_guid');
...
$allow_multiple_emails = false, $friend_guid = 0, $invitecode = '') {
	...
	// Create user
	$user = new Opspot\Entities\User();
	$user->username = $username;
	$user->setEmail($email);
	$user->name = $name;
	$user->access_id = ACCESS_PUBLIC;
	//$user->salt = generate_random_cleartext_password(); // Note salt generated before password!
	$user->password = Opspot\Core\Security\Password::generate($user, $password);
