 ...
 * MongoDB client
 */
namespace Opspot\Core\Data\MongoDB;
...
class Client implements Interfaces\ClientInterface
{
    ...
    public function __construct(array $options = [])
    {
        ...
        // $servers = isset($CONFIG->mongodb_servers) ?  $CONFIG->mongodb_servers : ['127.0.0.1'];
				// bad hack to handle docker - shreos
        $servers = isset($CONFIG->mongodb_servers) ?  $CONFIG->mongodb_servers : ['mongo'];
        $servers = implode(',', $servers);
        $options = [];
        $driverOptions = [];

				$this->db_name = isset($CONFIG->mongodb_db) ? $CONFIG->mongodb_db : 'opspot';
