 ...
 * @deprecated Use Opspot\Core\Events
 */
function elgg_register_event_handler($event, $object_type, $callback, $priority = 500) {
	return \Opspot\Core\Events\Dispatcher::register($event, "elgg/event/$object_type", $callback, $priority); // Register event with new system, but prefix it in the oldstyle namespace
	/*global $CONFIG;
...
function elgg_unregister_event_handler($event, $object_type, $callback) {
    return \Opspot\Core\Events\Dispatcher::unregister("elgg/event/$object_type", $event);
    /*
	global $CONFIG;
...
function elgg_trigger_event($event, $object_type, $object = null) {
    return \Opspot\Core\Events\Dispatcher::trigger($event, "elgg/event/$object_type", $object, true);
	/*global $CONFIG;
...
function elgg_register_plugin_hook_handler($hook, $type, $callback, $priority = 500) {
    return \Opspot\Core\Events\Dispatcher::register($hook, "elgg/hook/$type", $callback, $priority);
}
...
function elgg_unregister_plugin_hook_handler($hook, $entity_type, $callback) {
    return \Opspot\Core\Events\Dispatcher::unregister("elgg/hook/$entity_type", $hook, $callback);
}
...
function elgg_trigger_plugin_hook($hook, $type, $params = null, $returnvalue = null) {
    return \Opspot\Core\Events\Dispatcher::trigger($hook, "elgg/hook/$type", $params, $returnvalue);
	/*global $CONFIG;
...
function _elgg_php_exception_handler($exception) {
	...
	header('X-Opspot: Something is wrong', true, 500);
	...
	try {
		...
		$body = "<h1>Opspot | Automatic bug report</h1>\n";
		...
		//elgg_send_email('opspot@opspot.com', 'mark@opspot.com', 'Exception ' . get_class($vars['object']), nl2br(htmlentities(print_r($vars['object'], true), ENT_QUOTES, 'UTF-8')));
		if(function_exists('phpmailer_send')){
			phpmailer_send(
						...
						$body,
						null,
						//array('bill@opspot.com', 'john@opspot.com','mark@kramnorth.com'),
						true //html
			);
		}
...
function elgg_deprecated_notice($msg, $dep_version, $backtrace_level = 1) {
	...
	$elgg_version = Opspot\Core\Opspot::getVersion();
...
function elgg_add_action_tokens_to_url($url, $html_encode = FALSE) {
	...
	$query['__elgg_token'] = Opspot\Core\token::generate($query['__elgg_ts']);
