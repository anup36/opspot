...
class EntitiesBuilder
{
    ...
    public function build($row, $cache = true)
    {
        ...
        if (isset($row->subtype) && $row->subtype) {
            $sub = "Opspot\\Entities\\" . ucfirst($row->type) . "\\" . ucfirst($row->subtype);
            if (class_exists($sub) && is_subclass_of($sub, 'ElggEntity')) {
                return new $sub($row, $cache);
            } elseif (is_subclass_of($sub, "Opspot\\Entities\\DenormalizedEntity") || is_subclass_of($sub, "Opspot\\Entities\\NormalizedEntity")) {
                return (new $sub())->loadFromArray((array) $row);
            }
        }

        $default = "Opspot\\Entities\\" . ucfirst($row->type);
        if (class_exists($default) && is_subclass_of($default, 'ElggEntity')) {
            return new $default($row, $cache);
        } elseif (is_subclass_of($default, "Opspot\\Entities\\DenormalizedEntity") || is_subclass_of($default, "Opspot\\Entities\\NormalizedEntity")) {
