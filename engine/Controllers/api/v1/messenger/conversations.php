 ...
 * @author Mark Harding
 */
namespace Opspot\Controllers\api\v1\messenger;
...
use Opspot\Core\Messenger\Conversations;
...
class conversations implements Interfaces\Api
{
    ...
    public function post($pages)
    {
        ...
        foreach ($conversation->getParticipants() as $participant) {
            ...
            Core\Queue\Client::build()->setExchange("opspotqueue")
                                      ->setQueue("Push")
                                      ->send([
    ...
    public function put($pages)
    {
        ...
        switch ($pages[0]) {
            case 'call':
               \Opspot\Core\Queue\Client::build()->setExchange("opspotqueue")
                                                ->setQueue("Push")
                                                ->send(array(
                                                     "user_guid"=>$pages[1],
                                                    "message"=> \Opspot\Core\Session::getLoggedInUser()->name . " is calling you.",
                                                    "uri" => 'call',
                                                    "sound" => 'ringing-1.m4a',
                                                    "json" => json_encode(array(
                                                        "from_guid"=>\Opspot\Core\Session::getLoggedInUser()->guid,
                                                        "from_name"=>\Opspot\Core\Session::getLoggedInUser()->name
                                                    ))
                                                ));
                break;
            case 'no-answer':
              ...
              Core\Queue\Client::build()->setExchange("opspotqueue")
                                        ...
                                        ->send(array(
                                              "user_guid"=>$pages[1],
                                              "message"=> \Opspot\Core\Session::getLoggedInUser()->name . " tried to call you.",
                                              "uri" => 'chat',
